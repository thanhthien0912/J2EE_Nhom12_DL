...
function isLikelySlug(value: string) {
  return value.includes('-') && !/^[a-f0-9]{24}$/i.test(value);
}
...
export function Component() {
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      setLoading(true);
      try {
        const detailRes = await apiClient.get<ApiResponse<Product>>(
        ...
        const allRes = await apiClient.get<ApiResponse<PaginatedResponse<Product>>>(
        ...
        setRelated(
          allRes.data.data.content
            .filter((item) => item.brand === current.brand && item.id !== current.id)
            .slice(0, 4),
        );
      } catch {
        if (!isLikelySlug(id)) {
        ...
        try {
          ...
          const matched = allRes.data.data.content.find((item) => {
            const itemSlug = toSlug(item.name);
            return (
              itemSlug === id ||
              itemSlug.startsWith(`${id}-`) ||
              id.startsWith(`${itemSlug}-`)
            );
          });
          ...
          setRelated(
            allRes.data.data.content
              .filter((item) => item.brand === matched.brand && item.id !== matched.id)
              .slice(0, 4),
          );
          navigate(`/products/${matched.id}`, { replace: true });
        } catch {
          if (!active) return;
          setProduct(null);
          setRelated([]);
